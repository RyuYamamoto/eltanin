...
TEST(AStarBounds, DoesNotWrapAroundTheLeftEdge)
{
  ...
  EXPECT_FALSE(plan_raw(map, make_cost_model(), at_cell(map, 0, 1), at_cell(map, 3, 0)).has_value());
}
...
TEST(AStarBounds, DoesNotWrapAroundTheRightEdge)
{
  ...
  EXPECT_FALSE(plan_raw(map, make_cost_model(), at_cell(map, 3, 0), at_cell(map, 0, 1)).has_value());
}

TEST(AStarBounds, ConnectsWithinTheSameEdgeColumn)
{
  const Costmap map = split_two_row_map();
  const auto path = plan_raw(map, make_cost_model(), at_cell(map, 0, 0), at_cell(map, 0, 1));
...
TEST(AStarBounds, ExpandsTheOriginCellWithoutNegativeIndices)
{
  ...
  const auto path = plan_raw(map, make_cost_model(), at_cell(map, 0, 0), at_cell(map, 2, 2));
  ASSERT_TRUE(path.has_value());
...
TEST(AStarBounds, PlansOnASingleColumnGrid)
{
  ...
  const auto path = plan_raw(map, make_cost_model(), at_cell(map, 0, 0), at_cell(map, 0, 4));
  ASSERT_TRUE(path.has_value());
...
TEST(AStarBounds, PlansOnASingleRowGrid)
{
  ...
  const auto path = plan_raw(map, make_cost_model(), at_cell(map, 4, 0), at_cell(map, 0, 0));
  ASSERT_TRUE(path.has_value());
  EXPECT_EQ(path->size(), 5u);
  EXPECT_NEAR(path_length(*path), 4.0 * RESOLUTION, TOLERANCE);
  expect_grid_connected(*path, RESOLUTION);
...
TEST(AStarBounds, PlansOnASingleCellGrid)
{
  ...
  const auto path = plan_raw(map, make_cost_model(), at_cell(map, 0, 0), at_cell(map, 0, 0));
  ASSERT_TRUE(path.has_value());
