...
public:
    ...
    const Map & map, const Model & model, const Pose2D & start, const Pose2D & goal) const
  {
    ...
    const auto search_start =
      find_nearest_traversable(map, model, *start_index, params_.start_search_radius_cells);
    ...
    return plan_on_grid(
      PlanQuery{
        TraversabilityView{geometry, grid}, *search_start, *goal_index, effective_start, goal});
  }
