...
**ストール検出**: 前回再計画 (または開始) 以降の走行弧長が 0.20 m 未満なのに再び停止トリガが立ったら
`Outcome::Stalled` で終了する。その場旋回は弧長 0 だがトリガも立たないので誤検出しない。
...
統合テスト 7 件の合計 (直列 `ctest`):

| 構成 | 合計 | 最長の 1 件 |
|---|---|---|
| 既定 (`-O0` + `assert`) | 32.9 s | 9.5 s |
| ASan + UBSan | 94.6 s | 26.4 s |
...
| 7 | 参照地図が無い環境では実地図を使う 5 件がスキップされる。窓のスナップを固定する 2 件は常に走る |
