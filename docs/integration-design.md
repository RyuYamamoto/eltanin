...
| leg 1 | 427 姿勢 / 21.978 m | 381 姿勢 / 19.649 m |
| 総周期数 / シミュレーション時間 | 1622 / 81.10 s | 1670 / 83.50 s |
| 停止 | **しない** | する (注入した障害物まで 0.296 m) |
| 最終位置誤差 | 0.0835 m | 0.0834 m |
...
|---|---|---|---|
| leg 0 の経路 | 727 姿勢 / 37.246 m | 727 姿勢 / 37.246 m | 727 姿勢 / 37.246 m |
| leg 1 (迂回路) | — | 427 姿勢 / 21.978 m | 381 姿勢 / 19.649 m |
| 総周期数 | 1602 | 1622 | 1670 |
| 制限が効いた周期 | **0** | **0** | 15 |
| 衝突を予測した周期 | 18 | 23 | 35 |
| 最小 `collision_distance` | 0.8 m | 0.498 m | 0.2 m |
| 蓄積した観測点 | **0** | 25 | 25 |
| 再計画 | 0 | 1 (観測) | 1 (停止) |
| 全域 `update()` | 1 | 3 | 3 |
| 最終位置誤差 | 0.0883 m | 0.0835 m | 0.0834 m |
| 衝突した通過姿勢 | 0 | 0 | 0 |
| 経路からの最大距離 | 0.0463 m | 0.0326 m | 0.0351 m |

この表は `docs/planner-design.md` §13.14 (スムーザ既定重みの変更) の後に全行を測り直した。**変更前の記録値も
すでに古かった**: 本タスクの基点 `7f4b990` で走らせるとクリーンは 1603 周期 / 最終誤差 0.0992 m であり、
以前記録されていた 1568 周期 / 0.0237 m は再現しない。T7 の測定以降 (`0cd2642` の raytrace layer など) の
変更が閉ループ挙動を動かしている。「障害物までの最小フットプリント余裕」の行は本タスクで再測定して
いないため削除した。
