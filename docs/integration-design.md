...
選択中の planner で計画 → 新しい leg として追従を継続。`plan_leg()` は平滑化の分岐を持たない —
`plan_astar()` が既定で平滑化済みの経路を返すためである (`docs/planner-design.md` §13.6)。Hybrid A*
だけは、メモリ上限があるため生 A* 経路で切り出した corridor 内を探索する (§13.7)。上限 3 回。
