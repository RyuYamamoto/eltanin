...
(`test_collision_checker.cpp` の掃引で、膨張なしシナリオだけが差を出しているとおり)。
**食い違う姿勢の割合は実マップ・実 `local_map` に対して測っていない。**測るのは
`collision_predictor` が動く段階であり、その値が出るまでこの節に数字は置かない。
...
| クリアランスは**内接円**で測る | 当初は外接円を使い「精度より間違えようのなさ」を採ったが、実機で成立しなかった。kachaka は外接 0.266 m に対し半幅 0.120 m で、**幅 0.53 m 以下の廊下はどう設定してもランプの床に張り付く**。機体幅は 0.24 m なので 0.3 m の廊下でも通れるのに、0.3〜0.53 m の間で「狭いが通れる」と「接触寸前」を区別できない。実測では 0.5〜0.6 m の廊下で走行の大半が床値 0.25、指令は要求の 1/4 だった |
| 前方には楽観的でよい | 内接円は前方 (kachaka で 0.237 m) を 0.120 m として測るので、正面の障害物には楽観的な値になる。**近接ランプは減速だけを担当し、停止は制動距離則と反応時間則が実フットプリントで判定する**。ランプが覆うべきはロールアウトが通らない横方向であり、そこは内接円が正確な量を与える |
| 境界サンプリングはしない | 円 1 つで測る。精度を上げるより、値の意味が 1 行で言えることを採る |
