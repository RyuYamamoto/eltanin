...
自動選択の start / goal (727 点 / 37.3 m、§6.1 が測ったものと同じ経路)、既定パラメータ、`dt = 0.01 s`。

| 指標 | 実測 |
|---|---|
| 経路自身の非 `Free` 点 | **0 / 727** (プランナは仕様どおり) |
| 追従軌跡の最大横方向誤差 | **5.51 cm** |
| 1.62 cm を超えた区間 | **7 区間 / 走行 6.0 %**。すべて走行 28.5 m 以降 |
| 軌跡が `Circumscribed` 帯に入ったサンプル | **60 / 7538 (0.80 %)** |
| 軌跡が `Inscribed` 帯 (必ず衝突) に入ったサンプル | **0** |
| ゴール残距離 | 2.45 cm |

**合成円弧の 0.35 cm に対して実マップは 5.51 cm で、16 倍ある。** 原因は経路の曲率である。
この経路の曲率半径は**中央値 1.47 m、最小 0.133 m**で、63 個の折れ点のうち 24 個が 1.0 m を下回る。
先行距離 `L = 0.35 m` はこれらの半径より大きいので、Pure Pursuit は角を内側に切り (最大 5.51 cm)、
その後外側へ行き過ぎる (最大 4.98 cm)。1.62 cm 超の区間 456 サンプルの内訳は内側 248 / 外側 208 で、
**片側へのオフセットではなく角での振動**である。
