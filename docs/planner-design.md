...
**旋回箇所の振れ (bang-bang) は §13.12 で構造的に対処した。** 素の離散探索は primitive が直進・左 `1/R`・右 `1/R` の 3 本しかなく中間曲率を持たないため、斜め方向や緩い旋回を直進と旋回の交互で近似する。さらに 1 回の旋回 primitive の方位変化は `motion_step / R` = 0.177 rad (10.1°) で、方位ビン幅 `2π / 72` = 0.087 rad (5°) の 2 倍を動く。**最小の旋回操作がビン分解能を超えているため細かい方位調整ができず、横方向のずれを左右の weave で補正する** — これが振れの実体である。

解析接続の試行方針を変えることで、開けた地図では振れが完全に消える (§13.12)。障害物を迂回する区間では上記の量子化が残るため、単発の補正がわずかに残る。実測 (200 x 200、`resolution 0.05`、`resolution` の 7.5 m 分の壁を迂回):

```
旧   LLLL--L------RL-------RL----RR--------L----L--------RL------R------RR------LLLLLL   (反転 12 回)
新   LLLL------L----------------------RRRRRRRRRR---------------R-------------------LLLLLL   (反転 2 回、単発補正 2 箇所)
```

完全に消すには中間曲率の primitive を追加して 1 旋回ステップを 1 ビンに一致させる必要がある。これは successor 数が増え探索コストに直接効くので §13.15 に申し送る。
...
- 障害物の迂回区間に残る単発の方位補正 (§13.5)。中間曲率の primitive を追加して 1 旋回ステップを 1 方位ビンに一致させるのが筋だが、successor 数が増えて探索コストに直接効く。`steering_change_penalty` は既定の `motion_step` の下では効かないので、この目的には使えない
