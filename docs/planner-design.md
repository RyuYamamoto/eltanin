...
**出力契約は変わらない。** `free_goal_yaw` が緩めるのは「探索が達成しなければならないもの」であって、返す経路の末尾姿勢ではない。

1. まず**要求された goal 姿勢**へ解析接続を試みる。成功すればそこで終わりで、既定と同じ経路になる
2. 失敗し `free_goal_yaw` が true なら、goal *位置* へ最短の到達方位で接続する
3. どちらの場合も**末尾姿勢には要求された goal をそのまま載せる**

3 により、要求された向きが出力から消えることはない。2 を経た場合は末尾 1 区間が yaw の不連続になり、追従側がその場旋回で合わせる — これは **A\* が §6 で以前から採っている規約と同じ**である (A\* の末尾 yaw も接線ではなく要求値であり、実地図で最大 `|曲率|` 36 rad/m として現れるのはこの規約による)。膨張済み地図では goal セルが `Free` である以上その場旋回は必ず衝突しないので、この不連続は安全である。
