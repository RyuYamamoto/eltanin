...
| 末尾 yaw への進入 | 接線と不連続 (追従側がその場で合わせる) | 連続。ただし `free_goal_yaw` で運動学的に入れない場合は A\* と同じく不連続 |
| yaw の意味 | 経路接線 | 車体姿勢 |
...
### 13.15 goal 姿勢を自由にできるようにした (`free_goal_yaw`)

`eltanin_ros` から実地図で試したところ、`(5.0, 3.0)` へ yaw = 0 を要求したゴールが `Unreachable` になった。切り分けた結果:

| 条件 | 結果 |
|---|---|
| goal yaw 0 / 1.57 / 4.71 / 5.50 | **失敗** |
| goal yaw 2.36 / 3.14 / 3.93 | 成功 |
| corridor margin 30 → 240 セル (12 m) | すべて失敗 |
| `minimum_turning_radius` 0.40 → 0.10 | すべて失敗 |
| `minimum_turning_radius` 0.05 | 成功 |

corridor の広さでも start yaw でもなく、**要求された goal 姿勢 (位置 + yaw) へ前進のみの円弧で進入できない**ことが原因である。前進のみの Dubins は「その向きで到達せよ」という拘束であり、入れない向きが必ず存在する。

**差動二輪ではこの拘束は不要である。** その場旋回ができるので、最終 yaw は `control::GoalApproach` が合わせる。planner が最終姿勢まで担保する必要がない。

`HybridAStarParams::free_goal_yaw` (既定 false) を追加した。true のとき解析接続は goal *位置* を狙う。

**終端方位が自由なときは解くべき解析問題が変わる。** pose-to-pose の Dubins ではなく「点への最短 Dubins」であり、その最適解は **CS 型 (最小半径の円弧 1 本 + 直線)** である。終端で向きを合わせる必要がないので最後の円弧が要らない。到達方位は閉形式で求まる。

```
円の中心 c = p + turn * R * (左法線)          turn = ±1
d = |goal - c| ;  d < R ならその円では届かない
直線長 s = sqrt(d^2 - R^2)
離脱角 φ = atan2(goal - c) - turn * atan2(s, R)
到達方位 = φ + turn * π/2
```

2 つの円のうち弧長 + 直線長が短い方を採り、どちらも `d < R` のときだけノード自身の方位へ落とす。

最初の実装は「goal へのベアリング」と「ノードの方位」の 2 つを試すだけの近似だった。閉形式の最適解と総当たり (方位 2880 分割) で比べた実測:

| | 最適解より長い割合 | 超過長 平均 | 超過長 最悪 |
|---|---|---|---|
| 近似 (ベアリング + 自方位) | 81 % | 0.027 m | 2.15 m (3.12 倍) |
| **閉形式 (CS)** | **1.2 %** | **0.010 m** | 1.74 m (2.26 倍) |

残る 1.2 % は最適解が CS 型ですらない配置である。解析接続は §13.12 の間引きで多数のノードから試行され、**最初に成功したものを採る**設計なので、ノード単位の最適性が経路全体を決めるわけではない。総当たりや Reeds-Shepp の導入で潰せるが、費用に見合わないと判断した。

実地図での効果 (同じ start / goal、corridor margin 30):

| | goal yaw 厳密 (既定) | `free_goal_yaw = true` |
|---|---|---|
| goal yaw 8 通りの成功数 | 3 / 8 | **8 / 8** |
| 経路 | — | 158 姿勢 / 11.080 m |
| goal 位置への到達誤差 | — | **0.0000 m** |
| 最大 `\|曲率\|` | — | **2.50 rad/m** = `1 / minimum_turning_radius` |

goal 位置には厳密に到達し、曲率保証も保たれる。変わるのは末尾 yaw だけである。

**出力契約は変わらない。** `free_goal_yaw` が緩めるのは「探索が達成しなければならないもの」であって、返す経路の末尾姿勢ではない。**末尾姿勢には常に要求された goal をそのまま載せる。**

候補は 3 つある。要求された goal 姿勢、CS の到達方位、ノード自身の方位。これを**到達コスト**で並べ、衝突しない最初のものを採る。

```
到達コスト = そこまでの Dubins 長 + |到達方位 - 要求方位| * minimum_turning_radius
```

第 2 項は「要求された向きに合わせるための、その場旋回」を最小旋回半径での弧長に換算したものである。**要求 yaw の達成が安ければ運動学的に達成し、高くつくならその場旋回に落とす。**

この順序付けが無いと実害が出る。要求 yaw を無条件に優先した実装では、RViz のドラッグで付いた向きを達成するために遠回りのループを描いた。5 m 直進の位置にあるゴールでの実測:

| 要求 yaw | 要求 yaw を無条件優先 | 到達コストで選択 |
|---|---|---|
| 0.00 | 4.930 m / 旋回 0.00 rad | 4.930 m / 0.00 rad |
| 0.79 | 4.962 m / 0.66 rad | 4.962 m / 0.66 rad |
| 1.57 | 5.174 m / 1.57 rad | 5.174 m / 1.57 rad |
| **3.14** | **6.248 m / 3.29 rad** | **4.930 m / 0.00 rad** |

要求 yaw が進行方向から離れるほど連続到達の代償が大きくなり、π ではループになる。到達コストで選ぶと真後ろ向きの要求はその場旋回に落ちる。

その場旋回に落ちた場合、末尾 1 区間が yaw の不連続になる。これは **A\* が §6 で以前から採っている規約と同じ**である (A\* の末尾 yaw も接線ではなく要求値であり、実地図で最大 `|曲率|` 36 rad/m として現れるのはこの規約による)。膨張済み地図では goal セルが `Free` である以上その場旋回は必ず衝突しないので、この不連続は安全である。

実地図での確認 (start から 8 通りの goal yaw):

| | 修正前 | 修正後 |
|---|---|---|
| 末尾 yaw = 要求値 | 0 / 8 | **8 / 8** |
| goal 位置への到達誤差 | 0.0000 m | 0.0000 m |
| 運動学的に連続で入れた数 | — | 2 / 8 (残りは末尾がその場旋回) |

参考として、同じ配置で `minimum_turning_radius` を 0.05 まで下げても通るが、最大曲率が 21.82 rad/m になり Hybrid A\* を使う理由 (曲率保証) が失われる。A\* の平滑化済み経路が 35.97 rad/m であることと合わせると、`free_goal_yaw` + `R = 0.4` の 2.50 rad/m が最も筋の良い組み合わせである。
