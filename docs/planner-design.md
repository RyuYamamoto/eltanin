...
**出力契約の例外**: §13.5 は「Hybrid A\* の末尾姿勢は位置・yaw とも要求 goal に一致」と書いている。`free_goal_yaw = true` のときはこの yaw の項だけが例外になり、**位置は一致したまま、yaw は planner が選ぶ**。既定は false なので既存の契約は変わらない。
