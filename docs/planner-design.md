...
`HybridAStarParams::free_goal_yaw` (既定 false) を追加した。true のとき解析接続は goal *位置* を狙い、進入方位は次の 2 つを順に試す。

1. そのノードから goal へのベアリング (まっすぐ入る)
2. そのノード自身の方位 (今の向きのまま入る)

Dubins 解は 1 回の試行あたり最大 2 回で、全方位ビンを試す実装は採らなかった (解析接続は §13.12 で間引いているとはいえゴール近傍では毎ノード走るため)。
