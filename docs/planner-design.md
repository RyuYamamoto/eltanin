...
- **追従側が経路を実行できない。** これが最大の残件である。`turn_in_place` は間隔 0 の姿勢を、`reverse` は後退区間を出すが、`navyu_path_tracker` はどちらも扱えない。`emit_goal_rotation = false` は末尾だけの妥協で、途中の旋回には効かない。`eltanin::control` には開始時と終端の旋回はあるが、**経路途中の旋回と後退が無い**。ここが埋まるまで、今日入れた能力の大半は使えない
- **経路が区間の種別を持っていない。** 追従側は変位と車体向きの内積から前進 / 後退 / その場旋回を推測することになる。planner は確実に知っているので、捨てて推測させるのは筋が悪い。後退の可否は安全に直結する
