...
既定値は `weight_data = 0.1` / `weight_smooth = 0.4` / `tolerance = 1e-4` / `max_iterations = 100` (§13.14 で `0.5` / `0.3` から変更)。
...
**`assert` で守るのは収束条件のほうである** (`assert_smoother_params()`)。**既定値は両方を満たす**: 収束条件が `0.1 + 1.6 = 1.7 < 2`、凸結合条件が `0.1 + 0.8 = 0.9 <= 1.0`。したがって各更新は `(1 - wd - 2ws, wd, ws, ws) = (0.1, 0.1, 0.4, 0.4)` を係数とする現在点・元の点・両隣の凸結合であり、overshoot が原理的に起きない。

**旧既定 `0.5 / 0.3` は凸結合条件を満たさなかった** (`0.5 + 0.6 = 1.1 > 1.0`)。高周波モードが `A = -0.7` で符号反転しながら減衰するため、8 近傍 A* 経路の階段が符号反転を伴って残った。当時は「`0.5 / 0.3` は反復平滑化の既定値として広く使われている」ことを理由に維持したが、**グリッド探索の出力に対してはデータ項が保存すべき信号ではなく除去すべき人工物である**。§13.14 で変更した。
...
| 末尾 yaw への進入 | 接線と不連続 (追従側がその場で合わせる) | 制御集合の primitive で必ず連続 (`Differential` ではその場旋回、§13.15) |
| yaw の意味 | 経路接線 | 車体姿勢 |
| yaw の有限性 | 全姿勢で有限 | 全姿勢で有限 |
| 点間隔 | `resolution` 〜 `√2 * resolution` (平滑化は点数不変・端点固定なので上限が保たれる) | `(0, 1.5 * motion_step]`、同一経路内の最大/最小比は 1.5 以下。`Differential` ではその場旋回の区間だけ 0 になる (§13.15) |
...
**旋回箇所の振れ (bang-bang) は §13.12 と §13.13 で対処した。** 原因は 2 つある。

1. 解析接続を末尾 1 m でしか試していなかったので、その手前は離散探索の weave しか使えなかった (§13.12)
2. 1 回の旋回 primitive の方位変化 `motion_step / R` = 0.177 rad (10.1°) が方位ビン幅 `2π / 72` = 0.087 rad (5°) の 2 倍で、**最小の旋回操作がビン分解能を超えていた**。細かい方位調整ができないので、横方向のずれを単発の旋回で補正する (§13.13)

対処後、開けた地図では出力が最適 Dubins 経路と完全に一致し、障害物の迂回でも単発補正が 1 経路あたり 0.91 → 0.35 個に減った。実測 (200 x 200、`resolution 0.05`、7.5 m の壁を迂回):

```
旧        LLLL--L------RL-------RL----RR------L----L------RL----R----RR------LLLLLL
§13.12後  LLLL------L---------------------RRRRRRRRRR-----------R---------------LLLLLL
§13.13後  LLLLL---------------------------LLLLLLLRRRRRRRRRRRRRRRRR--------------LLLLLL
```

`-` の連なりに挟まれた単独の `L` / `R` が単発補正である。最終形では消えている。

参考として、**A\* の平滑化済み経路のほうが振れは大きい**。8 近傍のセル中心列に由来する 45° の折れが残るため、統合デモの同一経路で左右反転 70 回 (0.87 回/m)、最大 `|曲率|` 35.5 rad/m である (Hybrid A* は 2.503 rad/m = `1 / minimum_turning_radius` を弦長基準で厳密に守る)。追従側の実効的な滑らかさは `PurePursuit` の先行距離が吸収する。
...
§13.13 で primitive を 5 本にしたが、`nodes` は展開数比例で状態配列が支配項のままなので、下表のピーク RSS は変わらない。
...
|---|---|---|---|---|
| 15 (0.75 m) | 72 x 757 | 3.92e6 | 59 + 0 + 279 = 338 ms | 172 MB |
| **30 (1.5 m)** | 102 x 787 | **5.78e6** | 60 + 0 + 318 = **379 ms** | 172 MB |
| 60 (3.0 m) | 162 x 847 | 9.88e6 | 62 + 0 + 353 = 415 ms | 174 MB |
| 100 (5.0 m) | 242 x 927 | 1.62e7 | 62 + 0 + 380 = 443 ms | 221 MB |

出力経路はどの margin でも同一 (536 姿勢 / 37.800 m / 最大曲率 2.503 rad/m = `1 / minimum_turning_radius`) だった。margin は「guide からどれだけ離れた迂回を許すか」を決めるので、狭所での自由度と探索コストのトレードオフである。

**bounding box であることの限界**: 切り出しは guide の外接矩形なので、斜めに長い経路では矩形が大きくなる。`resolution 0.05` / `heading_bins 72` で 20e6 状態に収まる正方形は約 26 m 角である。これを超える配置では `StateSpaceTooLarge` になるので、margin を下げるか上限を上げる。密配列をハッシュへ移せばメモリが展開数比例になり、この制約自体を外せる — 次段の選択肢として申し送る。
...
- **出力**: `motion_step` を目安にした等分割で生成する。間隔は `length / count`。

分割数は `count = max(1, lround(length / motion_step))` である。`ceil` だと `length = 1.01 * motion_step` のときに `count = 2` となり間隔が `0.505 * motion_step` まで縮み、探索区間 (約 `motion_step`) との比が 1.98 に達してしまう。`lround` なら間隔は `[0.75, 1.5) * motion_step` に収まる。 `count == 1` (`length <= motion_step`) のときだけ末尾区間が極小になりうるので、**その区間が公称間隔の半分未満なら 1 つ前の姿勢を落とす**。落とす姿勢は衝突検査済みの点なので経路の妥当性は変わらず、統合後の最終区間は `1.5 * motion_step` 未満に収まる。これにより経路全体の最大/最小比は 1.5 以下になり、§13.5 の契約として書ける。

実測 (`open200` / 既定パラメータ):

| | 旧 | 新 |
|---|---|---|
| 隣接点間隔 | [0.0244, 0.0500] m | [0.0709, 0.0709] m |
| 最大/最小比 | 2.05 | **1.00** |

開けた地図 960 配置の掃引では最大/最小比の最悪値が **1.0014**、最大間隔が `1.021 * motion_step` である。

`dubins->length() == 0.0` のとき末尾を要求 goal 姿勢へ差し替える旧来の振る舞いは維持している。

### 13.12 解析接続を残距離に応じた間引きで試すようにした

旧実装は解析接続 (Dubins) を `dubins_expansion_distance` (既定 1.0 m) 以内のノードでしか試さなかった。これが §13.5 の振れの主因である。**解析接続は曲率制約を満たす最短の前進経路そのものなので、衝突しないなら離散探索の weave より常に良い。** 試す機会が末尾 1 m に限られていたために、その手前は量子化された方位で weave するしかなかった。

一律に `dubins_expansion_distance` を広げるのは誤りである。実測すると壁の迂回で**遅くなり品質も落ちる**。

| `dubins_expansion_distance` | turn の反転 / 無駄旋回 | wall の無駄旋回 | wall の所要 |
|---|---|---|---|
| 1.0 m (旧既定) | 2 / 0.787 rad | 3.654 rad | 753 ms |
| 10 m | **0 / 0.000 rad** | 5.999 rad | **5381 ms** |

全ノードで Dubins 解と曲線全体の衝突検査を走らせるコスト (wall は 71 万展開) と、最初に接続できたノードが良いノードとは限らないことが原因である。

**採った方針**は Dolgov / Thrun および nav2 SmacPlanner と同じ間引きである。

1. **最初のノードでは必ず試す。** 開けた地図では start からの Dubins 経路が答えそのものなので、1 回の Dubins 解で最適解が出る。
2. `dubins_expansion_distance` 以内では従来どおり毎ノード試す。
3. それより遠いノードでは `floor(ゴールまでのセル数 / analytic_expansion_ratio)` 回に 1 度だけ試す。遠いほど間隔が広く、近づくほど詰まる。

`analytic_expansion_ratio` の既定 1.0 は実測で選んだ。

| `analytic_expansion_ratio` | turn の無駄旋回 | wall の無駄旋回 | wall の所要 | split (到達不能) の所要 |
|---|---|---|---|---|
| 実質無効 (0.001) | 0.000 rad | 3.595 rad | 854 ms | 1494 ms |
| **1.0 (既定)** | **0.000 rad** | **3.554 rad** | **748 ms** | **1498 ms** |
| 3.5 | 0.000 rad | 5.349 rad | 822 ms | 1783 ms |
| 50.0 | 0.000 rad | 6.377 rad | 1953 ms | 4512 ms |

1.0 より大きくすると、迂回の途中で早すぎる接続を掴んで品質が落ちる。到達不能ケースの所要時間も増える。1.0 は wall を悪化させず (むしろ僅かに改善)、到達不能ケースのコストも増やさない。

**効果**: 開けた地図 960 配置 (goal 位置 15 通り × goal 方位 8 通り × セル内オフセット 5 通り) で、出力が**最適 Dubins 経路と 100 % 一致**する。

| | 修正前 | 修正後 |
|---|---|---|
| 最適 Dubins と一致した割合 | 9.9 % | **100 %** |
| 超過旋回 (平均 / 最悪) | 0.935 / 4.582 rad | **0.000 / 0.000 rad** |
| 経路長 / 最適 Dubins 長 (最悪) | 1.812 | **1.000** |

統合デモの実経路 (実地図、2 レグ、約 80 m) では総旋回量が 17.694 rad (旧実装) → 17.685 rad (§13.7/§13.9 の時点) → **14.014 rad** に、平均 `|曲率|` が 0.429 → 0.267 → **0.211 rad/m** に下がった。追従の最大経路逸脱も 0.251 m → 0.061 m に改善している。メモリ・到達不能ケースの所要時間は変わらない。

`HybridAStarPlanner.ReturnsTheOptimalDubinsPathOnAnOpenMap` がこの性質を回帰として固定する。

### 13.13 中間曲率の motion primitive を追加した

§13.12 で開けた地図の振れは消えたが、障害物を迂回する区間には単発の方位補正が残った。原因は §13.5 の 2 番目、**1 旋回 primitive が方位ビンの 2 倍動く**ことである。既定 (`motion_step = √2 * resolution = 0.0707`、`R = 0.4`、`heading_bins = 72`) では 1 旋回が 0.177 rad、ビン幅が 0.087 rad なので、方位を 1 ビンだけ寄せる操作が存在しない。

**先に否定した対案**: `heading_bins` を 1 旋回分に合わせる (`2πR / motion_step` = 35.5 → 36)。ビン幅と旋回量が一致すれば overshoot が消えるはずだが、実測では単発補正が 0.91 → 0.72 個/経路にしか減らず、総旋回量は 5.330 → 6.059 rad に増えた。逆に `heading_bins = 144` にすると経路は直線的になる (総旋回 4.899 rad) が単発補正は 1.06 個に増える。**細かいビンほど経路は直線的になるが単発補正は増える**という逆相関があり、ビン数の調整では両立しない。

**採った方針**: primitive を 3 本から 5 本にし、曲率 `±1 / (2R)` の緩旋回を加える。緩旋回 1 回の方位変化は 0.088 rad で既定のビン幅 0.087 rad とほぼ一致するので、**方位を 1 ビンだけ寄せる操作が存在するようになる**。
...
`steering_penalty` は曲率の大きさに比例させない。比例させると緩旋回が「安く曲がる手段」になり、探索が緩い大回りを選んで総旋回量が 5.660 rad まで増えた。**どの曲率でも一律**にした結果 5.685 rad で、比例版と大差ないうえ探索が速い。`steering_change_penalty` は曲率の変化量 `|Δscale|` に比例させる (3 本のときの「操舵が変わったら加算」と部分集合上で一致する)。

§13.9 の進行条件は変更不要である。弦長は曲率が大きいほど短いので、最も鋭い `1/R` について検査すれば緩旋回も満たす。

実測 (障害物レイアウト 80 通り = 壁の高さ 5 × 壁の位置 4 × goal 方位 4):

| | 3 本 | 5 本 |
|---|---|---|
| **単発補正 (平均/経路)** | 0.91 | **0.35** |
| 総旋回量 | 5.330 rad | 6.140 rad |
| 経路長 | 13.623 m | 13.647 m |
| 探索時間 | 558 ms | 868 ms |

**受け入れたコスト**: successor が 5/3 倍になるので探索が約 1.5 倍遅い。これを一部取り戻すため、**successor の受理判定の順序を入れ替えた**。従来は `collision_free_successor()` で primitive 全体の衝突検査を払ってから closed / `g_score` で捨てていた。終端姿勢を `propagate()` で 1 回求めて状態と暫定コストを先に判定し、それらを通ったものだけ内部サンプルを検査する。採用する successor の集合は変わらないので出力は同一である (80 レイアウトの全指標が一致することで確認した)。

| | 3 本 (順序変更なし) | 5 本 + 順序変更 |
|---|---|---|
| wall (到達可能な迂回) | 812 ms | 1098 ms |
| **split (到達不能、A-7 の判定)** | 1501 ms | **1837 ms** |
| ピーク RSS (open200 / split) | 25.5 / 121.8 MB | 25.4 / 121.7 MB |

メモリは変わらない (`nodes` は展開数比例だが状態配列が支配項のまま)。**A-7 の 2 s は満たすが余裕が 25 % から 8 % に減った。** より遅い機械や `heading_bins` を増やした設定では A-7 を割りうるので、その場合は `max_expansions` を下げるか primitive を 3 本へ戻す (`MOTIONS` の後 2 要素を削るだけ) こと。

統合デモ (実地図、corridor 切り出し) では全指標が改善した。

| | 3 本 | 5 本 |
|---|---|---|
| leg 1 の経路長 | 22.0794 m | **22.0133 m** |
| 総旋回量 (2 レグ計) | 14.014 rad | **13.330 rad** |
| 平均 `\|曲率\|` | 0.211 rad/m | **0.198 rad/m** |
| 追従の最大経路逸脱 | 0.0606 m | **0.0519 m** |
| 最終位置誤差 | 0.0961 m | **0.0864 m** |
| corridor の探索時間 | 2.1 ms | 2.2 ms |

運用形態 (corridor 切り出し) では探索時間の増加が現れず、追従品質が上がる。探索時間 1.5 倍が出るのは 200 x 200 全面の広い迂回に限る。

### 13.14 スムーザの既定重みを変えた (A* 経路のうねりの実体)

Hybrid A* を §13.12 / §13.13 で直したあとも「経路が曲がる箇所でうねうねする」という報告が続いた。調べると**うねりは A\* 側**にあり、Hybrid A* の修正では届いていなかった。統合デモの同一区間 (leg 0、arc 27.5〜34.5 m) を測ると差は明白である。

| | A\* (旧既定) | Hybrid A\* |
|---|---|---|
| 最大 `\|曲率\|` (末尾区間を除く) | 8.42 rad/m | 2.51 rad/m |
| 曲率符号の反転 | 10 | 5 |

`eltanin_ros` の既定プランナは A* なので、報告されていたのはこちらである。

**原因はスムーザの既定重みであり、平滑化がほとんど効いていなかった。** 反復回数を 100 → 2000 にしても `tolerance` を 0 にしても出力が 1 mm も変わらないので、収束はしている。不動点が元の階段のすぐ近くにあるのが問題で、その理由は §8.3 の凸結合条件 `weight_data + 2 * weight_smooth <= 1` を旧既定 `0.5 / 0.3` が満たしていなかったことにある (`1.1 > 1.0`)。高周波モードの増幅率が `A = -0.7` と負なので、8 近傍経路の階段が符号反転を伴って残る。

重みを振った実測 (実地図 4000 x 4000、A* 生経路 727 点 / 37.398 m):

| `weight_data` / `weight_smooth` | 経路長 | 最大 `\|曲率\|` (内部) | 符号反転 | roughness | 障害物内の点 |
|---|---|---|---|---|---|
| 生 A* (平滑化なし) | 37.398 m | 15.71 rad/m | 10 | 0.6000 | 0 |
| 0.5 / 0.30 (旧既定) | 37.321 m | 8.42 rad/m | 10 | 0.3522 | 0 |
| 0.2 / 0.40 | 37.279 m | 5.05 rad/m | 6 | 0.2625 | 0 |
| **0.1 / 0.40 (新既定)** | 37.246 m | **3.61 rad/m** | **6** | **0.2373** | 0 |
| 0.05 / 0.45 | 37.213 m | 7.30 rad/m | 6 | 0.2091 | 0 |

`0.05 / 0.45` まで弱めると roughness は下がるが最大曲率が悪化する。データ項がほぼ無くなると経路が直線へ潰れようとし、障害物で候補が却下された点だけが取り残されて新しい折れになるためである。`0.1 / 0.4` が両方を最良にする点であり、§8.3 の凸結合条件も満たす。

**判断の根拠**: グリッド探索の出力に対して、データ項が守っている「元の経路」は**セル中心列という人工物そのもの**である。守るべき信号ではない。強いデータ項は Hybrid A* のような連続経路や外部由来の経路には意味があるが、A* 経路には逆効果である。

経路品質 (`examples/eltanin_track_on_real_map`、同一 start/goal):

| | 旧既定 | 新既定 |
|---|---|---|
| 経路長 | 37.321 m | 37.246 m |
| `\|曲率\|` の中央値 (全 725 内点) | 0.000 rad/m | 0.000 rad/m |
| `\|曲率\|` の p99 / 最大 | 6.044 / 7.459 rad/m | **2.491 / 3.467 rad/m** |
| 総旋回量 `∫\|曲率\| ds` | 5.812 rad | **3.981 rad** |
| 厳密に直線な内点 | 662 / 725 | 619 / 725 |
| 追従の最大横方向誤差 | 5.51 cm | **4.93 cm** |
| 軌跡が `Circumscribed` 帯に入ったサンプル | 60 / 7538 | 82 / 7531 |
| 経路自身の非 `Free` 点 | 0 | 0 |

**効果は曲率の裾に限られる。** 中央値はどちらも 0 で、経路の大半は厳密な直線区間のままである。改善するのは折れの鋭さ (p99 / 最大で約 2.2 倍緩い) と総旋回量 (-31 %) であり、**「直線区間が離散的な折れで繋がった折れ線」という経路の性質は変わらない。** 拡大して見たときの見た目がほとんど変わらないのはこのためである。

**曲率半径の「中央値」で比較してはならない。** 非共線な点だけを母集団に取ると平滑化で母集団自体が 63 → 106 点に変わるため、見かけ上 1.473 → 3.866 m と大きく改善して見えるが、これは母集団の違いによる artifact である。

**受け入れたコスト**: `Circumscribed` 帯に入るサンプルが 60 → 82 に増える。角を内側に切るようになるためである。`Inscribed` (必ず衝突) は 0 のままで、経路自身の非 `Free` 点も 0 である。これは `docs/costmap-design.md` の「`Circumscribed` は姿勢によっては衝突しうる帯」という契約の範囲内だが、余裕は減っている。

**A-8 を意図的に破った。** 「`plan_astar()` の既定出力が旧 `plan()` + 旧 `smooth()` とビット同一」という受け入れ条件は、既定値を変える以上成り立たない。生 A* 経路 (`smoother = nullopt`) の期待経路長テストは維持されており、`plan_astar()` の既定出力が「生経路 + 既定 `smooth()`」と一致することも維持されている。

**この変更以前から `docs/integration-design.md` §12 の記録値は古かった。** 本タスクの基点 `7f4b990` で統合デモを走らせても、記録されたクリーン時 1568 周期 / 最終誤差 0.0237 m は再現せず 1603 周期 / 0.0992 m だった。T7 の測定以降 (`0cd2642` の raytrace layer など) の変更が閉ループ挙動を動かしている。§12 の表は本タスクで全行を測り直した。

### 13.15 運動モデルを制御集合として宣言するようにした

`eltanin_ros` から実地図で試したところ、`(5.0, 3.0)` へ yaw = 0 を要求したゴールが `Unreachable` になった。切り分けると corridor でも start yaw でもなく、**要求された goal 姿勢へ前進のみの円弧で進入できない**ことが原因だった。

ここから「常に成功する / 要求 goal 姿勢に到達する / 遠回りしない / 運動学的に連続」の 4 つが両立しない状態に入り、どれを捨てるかを 4 回往復した。最後は末尾に要求 yaw を刻印する形にしたが、**それは宣言したモデル (前進のみ・最小旋回半径) では生成できない動作**であり、自分の契約に違反した経路を返していた。

**原因はモデルの不一致である。** Kachaka は差動二輪でその場旋回できるのに、Hybrid A\* は車両型 (Ackermann) を仮定していた。最小旋回半径を課している理由は運動学ではなく「経路を滑らかにしたい」からで、**ソフトな選好をハードな拘束として表現していた**。上の 4 つが両立しないのはその帰結である。

#### 契約を検査可能にする

> **出力経路の隣接する 2 姿勢は、宣言した制御集合の primitive ちょうど 1 本で接続される。**

`expect_follows_control_set()` がこれを検査する。導入時に実際の違反を検出した (末尾区間が弦長 0.14 m と旋回 π を同時に持ち、円弧でもその場旋回でもなかった)。

#### 制御集合

primitive を **(travel, turn) = (符号付き移動量, 符号付き方位変化)** に一般化した。`propagate()` は 3 ケースを扱う。

| primitive | travel | turn |
|---|---|---|
| 直進 | `+L` | 0 |
| 左 / 右旋回 | `+L` | `±L/R` |
| その場旋回 | **0** | `±ビン幅` |
...
- `Dubins` (既定) — 直進 + 左右旋回。車両型
- `Differential` — その場旋回を追加。差動二輪

後退 (Reeds-Shepp) は `travel < 0` としてこの枠にそのまま載る。`propagate()` の円弧積分は travel の符号に依らず厳密なので、追加の場合分けは要らない。

その場旋回のコストは `|turn| * minimum_turning_radius` — 同じ方位変化を最小半径の円弧で行った場合の弧長である。位置が進まないぶん `h` が下がらないので、探索は必要なときしか使わない。膨張済み地図では `Free` セルでの旋回は必ず衝突しないため、追加の衝突検査も要らない。

#### 終端の扱い

解析接続は要求 goal 姿勢を優先し、`Differential` のときは「goal 位置への最短到達方位 + その場旋回」も候補にして**到達コスト**で選ぶ。

```
到達コスト = そこまでの Dubins 長 + |到達方位 - 要求方位| * minimum_turning_radius
```

これが無いと、要求 yaw を無条件に優先した実装が RViz のドラッグで付いた向きを達成するためにループを描いた (5 m 直進のゴールで、yaw = π のとき 4.930 m → 6.248 m / 旋回 3.29 rad)。

その場旋回は**独立した姿勢として出力する**。刻印して 1 区間に潰すと、円弧と旋回が混ざって制御集合の primitive でなくなる。
...
|---|---|---|
| 成功 | 3 / 8 | **8 / 8** |
| 走行区間の最大 `\|曲率\|` | 2.50 = 1/R | **2.50 = 1/R** |
| goal 位置への到達誤差 | 0.0000 m | **0.0000 m** |
| 末尾 yaw | 要求値 | **要求値** |
| その場旋回の回数 | 0 | 1〜2 |
| 経路長 | 11.20 m | 11.48 m (+2.5 %) |
| 所要 | 78〜99 ms | 234〜320 ms |

**4 つの要求がすべて、モデル上正しい形で成立する。** `Differential` が 3〜4 倍遅いのは primitive が 3 → 5 本になるためで、これは正しいモデルの対価である。

#### 出力契約の変更

`Differential` では**隣接姿勢の間隔が 0 の区間**が現れる。これはその場旋回であり、§13.5 の「点間隔 `(0, 1.5 * motion_step]`」は `Dubins` にのみ適用される。追従側がこれを解釈できることが前提になる。

#### 撤去したもの

`free_goal_yaw` は「モデルが足りないことの回避策」だったので削除した。`Differential` を選べば要求姿勢へ常に到達できる。

### 13.16 障害物を考慮したヒューリスティックを入れた

`eltanin_ros` から 36 m 先へ計画したところ `ExpansionLimitReached` になった (4.1 s)。`max_expansions` を外すと 4846 ms で成功したので経路は存在し、打ち切りに掛かっていただけである。corridor を margin 4 まで絞っても 43 m の guide の外接矩形が支配的で効かなかった。

原因は §13.10 に「残る限界」として記録したとおり、**ヒューリスティックがユークリッド距離で障害物を無視する**ことである。D-1 で範囲外としていたが、実運用で計画が通らない以上やらざるを得ない。

**実装**: 探索の前に goal セルから 8 近傍 Dijkstra を 1 回走らせ、セル上の goal までの距離場 `cost_to_go` を作る。角の抜けは A\* と同じく禁止するので、2 つの探索が「繋がっている」と見なす範囲が一致する。ヒューリスティックは

```
h(pose) = max(ユークリッド距離, heuristic_weight * cost_to_go[pose のセル])
```

とする。距離場は `float` 1 本 (セル数 × 4 byte) で、状態配列の 8.125 B/state に比べれば無視できる。メモリ判定にも加算した。

**到達不能セルの枝刈り**: `cost_to_go` が無限大のセルへ進む successor は捨てる。goal へ到達しようがないセルに状態を割り当てない。分断された地図では start 側が全て無限大になるので、探索は即座に尽きる。

**`heuristic_weight` を 0.8 にした理由**: オクタイル距離は連続最短経路を最大 8 % 過大評価する (45° 刻みしか使えないため)。重み 1.0 では非許容になり探索が貪欲化して、経路が距離場の階段構造をなぞって振れる。実測 (実地図、42 m の経路):

| `heuristic_weight` | 経路長 | 総旋回 | 曲率反転 | 単発補正 | 所要 |
|---|---|---|---|---|---|
| 0.0 (従来のユークリッドのみ) | 41.71 m | 5.24 rad | 6 | 3 | 4846 ms |
| **0.80 (既定)** | 41.76 m | 5.08 rad | **6** | **2** | **2564 ms** |
| 0.90 | 42.01 m | 7.94 rad | 16 | 15 | 1717 ms |
| 1.00 | 42.18 m | 11.03 rad | **28** | **21** | 96 ms |

0.8 は**品質を従来と同一に保ったまま 1.9 倍速い**。1.0 まで上げると 50 倍速くなるが目に見えて振れる。速度が要る利用者のために `HybridAStarParams::heuristic_weight` として公開した。

**効果**:

| ケース | 従来 | 現在 (0.8) |
|---|---|---|
| split 200 x 200 (到達不能、A-7) | 1369 ms | **3.9 ms** |
| wall 200 x 200 (迂回) | 1002 ms | 315 ms |
| 実地図 42 m | **打ち切り失敗** | **2564 ms で成功** |
| 実地図 10 m | — | 57 ms |
| 障害物 80 レイアウト | 368 ms | 257 ms |
| ピーク RSS (open200) | 25.5 MB | 25.8 MB |

**A-7 の余裕が 8 % から 500 倍になった。** §13.13 で薄くなった余裕はこれで解消した。到達不能ケースが速いのは枝刈りが効くためで、`heuristic_weight` に依らない。

開けた地図の最適性 (960 配置で最適 Dubins と 100 % 一致) と点間隔の契約は変わらない。

**副作用**: 分断された地図では展開上限に達する前に `Unreachable` を返すようになった。`PlannerErrors.DistinguishesTheExpansionLimitFromUnreachability` は「到達可能だが上限に掛かる」配置に作り替え、分断は `ASplitMapIsUnreachableRatherThanCutOffByTheLimit` で別に固定した。
...
- 障害物の迂回区間に残る単発の方位補正 (§13.13 で 0.91 → 0.35 個/経路まで減らしたが 0 ではない)。primitive をさらに増やすと探索コストが線形に増えるので、曲率制約を保つ後処理 (conjugate gradient 系の平滑化) の方が筋が良い。`steering_change_penalty` は既定の `motion_step` の下では効かないので、この目的には使えない
- 増分プランナ (D\* Lite) 用の別 I/F
- 探索作業領域を保持するプランナ型への移行 (`plan()` の `const` を外す破壊的変更)

---

## 14. T9: Reeds-Shepp、クリアランス、狭所フォールバック

`.plait/03-design-review.md` §4 に残った D-3 / D-4 / D-7 / D-8 を実施した記録である。基点は `2748c7c`。

### 14.1 旋回半径の上限が消えた (D-8)

`motion_step` の自動値を `√2 * resolution` から **`max(√2 * resolution, R * bin_width)`** に変えた。

旧実装では自動値が `√2 * resolution` 固定だったため、§13.9 の旋回条件 `motion_step / R >= bin_width` が `R <= motion_step / bin_width` を要求し、**既定 (`resolution = 0.05`, `heading_bins = 72`) では `R > 0.81 m` が開けた地図でも `ParamsIncompatibleWithMap` になっていた**。乗用車型の 1.0 m 級はすべて計画できなかった。

新しい自動値は旋回条件を構成上必ず満たす。`R * bin_width >= √2 * res` のときは `motion_step / R = bin_width`、そうでないときは `motion_step = √2 * res` かつ `R < √2 * res / bin_width` なので `motion_step / R > bin_width` である。既定 (`R = 0.4`) では `R * bin_width = 0.035 < 0.0707` なので**自動値は変わらず、既存の出力はビット同一**である。

明示指定に対する §13.9 の検査は変えていない。

| `R` [m] | 旧 | 新 (`resolution` 0.05、開けた 10 m 角) |
|---|---|---|
| 0.4 / 0.8 | 成功 | 成功 (経路長・点数とも同一) |
| 0.82 / 1.0 / 1.5 / 2.0 / 3.0 | **`ParamsIncompatibleWithMap`** | 成功 |

回帰テストは `HybridAStarPlanner.PlansAtRadiiOneTurnCannotResolveIntoAHeadingBin`。

### 14.2 Reeds-Shepp を実装し、制御集合に載せた (D-7)

`reeds_shepp_path.hpp` は `28864ed` で**宣言だけが入り、定義もテストも利用箇所も無い状態**で配布されていた (`install(DIRECTORY include/)` の対象なので、利用者が呼ぶとリンクエラーになる)。実装した。

`src/planner/reeds_shepp_path.cpp` は Reeds と Shepp (1990) の 18 語 48 経路を、恒等 / time flip / 反転 / 両方の 4 対称と「goal 側から読む」変換で覆う。基本式 8.1〜8.11 を 1 本ずつ関数にし、対称の適用を `VARIANTS` の表で回している。

検証は不変量で行った (20 万組の乱数姿勢対):

| 主張 | 実測 |
|---|---|
| 前進のみの Dubins 経路より長くならない | 違反 0 件 |
| `d(a,b) == d(b,a)` (Dubins は非対称、RS は対称) | 最大差 1.2e-14 |
| 剛体変換で不変 | 最大差 1.2e-14 |
| 直線距離を下回らない | 違反 0 件 |
| 1 セグメント内の曲率が `1/R` 以下 | 最大 0.769237 (上限 0.769231、差は差分近似の誤差) |

`MotionModel::ReedsShepp` は前進 3 本に後退 3 本を足した 6 本の制御集合を使い、解析接続を Reeds-Shepp 解に差し替える。
...
| 配置 (`resolution` 0.05、開けた地図) | Dubins | Differential | ReedsShepp |
|---|---|---|---|
| goal が真後ろ 2 m・同じ向き | 4.510 m | 4.510 m | **2.000 m** (全区間後退) |
| 0.8 m 幅の行き止まりで反転 | **失敗** (`Unreachable`) | 3.258 m (その場旋回) | 3.455 m (切り返し 9 回) |

### 14.3 クリアランス距離場を縫い目の内側に作った

D-3 と D-4 はどちらも「このセルは壁からどれだけ離れているか」を要る。3 値の分類グリッドはこれを潰してしまうので、**分類済みグリッドから距離変換を作る**ことにした。`Map` / `Model` の縫い目 (§2) は広げていないので、**同じ分類を返すコストマップと距離場は同じ経路を出す**という縫い目の主張はそのまま保たれる (`PlannerSeam` のテスト群が通る)。

Felzenszwalb と Huttenlocher の分離可能な厳密ユークリッド距離変換で、総当たり探索と 5.4e-8 m 以内で一致する。列方向のパスは 32 セル角のタイル転置を挟んで連続アクセスにしている。

**種は「実障害物セル」であって「非 Free セル」ではない。** 当初は非 Free を種にしていたが、§14.7 で膨張帯に入れるようにした結果、帯の中がすべて「クリアランス 0」になり**帯の中での良し悪しが区別できなくなった**。壁からの距離で差がつかないので、狭路の中心を選ぶ理由が無い。分類グリッドに実障害物ビットを載せてある (§14.7) ので、そこから測る。

型は増やしていない。計算結果は `map::DistanceMap` そのもの (`ClearanceMap` はその別名) で、補間と勾配は自由関数 `clearance_at()` / `clearance_gradient()` である。`DistanceMap` (入力) と `ClearanceMap` (計算した余裕) は用途で読み分ける。

**地図の縁は障害物として扱わない。** これは実装中に一度間違えた箇所である。当初は「地図の外は走れないのだから縁も壁」としていたが、Hybrid A* は**切り出した corridor 上で走る**ので、corridor の人工的な縁がすべて壁になり、統合デモで Hybrid A* がゴールに到達しなくなった (最終誤差 18.5 m、`replan_limit`)。縁は「そこで corridor を切っただけ」であり、地図外へ出られないことは `TraversabilityView::traversable()` が既に保証している。

### 14.4 探索がクリアランスを払うようにした (D-4)

`ClearanceCost{penalty, distance}` を A* と Hybrid A* の双方に持たせた。進んだ距離 1 m あたり `penalty * max(0, 1 - clearance / distance)` を加算する。**距離あたりに課すので、ヒューリスティック (純粋な距離) は下界のままで許容性を失わない。**

`docs/planner-design.md` §1 の「コスト値を経路コストに加算する重み付け探索はやらない」を**部分的に撤回した**。当時の理由は「期待長がパラメータ依存になり最適性テストの基準が失われる」だった。これに対する答えは、**生 A* 経路 (`raw_astar_params()`) がクリアランス整形も切ることで、オクタイル距離の解析値を主張するテスト群はそのまま生きている**ことである。既定経路は「最短」ではなく「壁から離れた短い経路」であると契約を書き換えた。

**既定値は左右で違う。理由は距離場の構築コストである。**

| | 既定 | 根拠 |
|---|---|---|
| `AStarParams::clearance` | `{0.0, 0.5}` = 切 | A* は実地図全面 (4000×4000) を走るので距離場が 461 ms かかる。計画 1 回が 144 → 428 ms になり N-5 (A* を遅くしない) を破る |
| `HybridAStarParams::clearance` | `{0.3, 0.2}` = 入 | Hybrid A* は corridor (102×787) でしか走らないので距離場は無視できる |

実地図での実測 (`penalty` を上げるほどクリアランスは伸びるが、A* は全面距離場の代金を払う):

| A* 設定 | 経路長 | クリアランス p05 | 平均 | 所要 |
|---|---|---|---|---|
| クリアランス切 (既定) | 37.361 m | 0.132 m | 0.292 m | **105 ms** |
| `{1.0, 0.5}` (opt-in) | 37.281 m | **0.299 m** | **0.513 m** | 428 ms |

corridor 上の Hybrid A*:

| Hybrid 設定 | 経路長 | クリアランス p05 | 平均 | 所要 |
|---|---|---|---|---|
| 切 | 37.755 m | 0.081 m | 0.358 m | 242 ms |
| **`{0.3, 0.2}` (既定)** | 37.789 m | **0.172 m** | 0.405 m | 214 ms |
| `{1.0, 0.5}` | 38.049 m | 0.244 m | 0.493 m | 273 ms |

`{1.0, 0.5}` を Hybrid の既定にしない理由は経路長ではなく**統合デモが通らないこと**である。注入した障害物の脇の隙間が `d0 = 0.5 m` を割るため、そこを避けて 60 m の大回りを選び、結局通れずに `replan_limit` に達した。**クリアランスを強く要求するほど、狭所を通れるのに通らなくなる。**

### 14.5 平滑化に曲率項と障害物項を入れた (D-3)

`SmootherParams` に `weight_curvature` (既定 0.01) と `weight_obstacle` / `obstacle_distance` (既定 0.05 / 0.3) を足した。

- **曲率項**は離散曲げエネルギー `Σ ½|p_{i+1} - 2p_i + p_{i-1}|²` の勾配、すなわち 5 点ステンシル `6p_i - 4p_{i-1} - 4p_{i+1} + p_{i-2} + p_{i+2}` である。両端から 2 点目までは片側の隣接が足りないので適用しない。Dolgov の曲率項 (区間長で正規化する形) ではなく曲げエネルギーを選んだのは、勾配が厳密かつ有限差分で検査できるからで、`PathSmoother.TheBendingStencilIsTheGradientOfTheEnergyItDescends` がそれを固定している。
- **障害物項**は `½(d0 - d)²` の勾配 `-(d0 - d)∇d` である。距離場はセル単位に量子化されているので、**双線形補間**した値と勾配を使う。補間なしだと勾配が階段状になり、経路が波打った (実地図で曲率の符号反転 129 回)。
- 収束条件を `wd + 4*ws + 16*wc < 2` に更新した。曲げ演算子の最大固有値が 16 だからである。

**距離場は経路の周りの窓だけで作る。** 平滑化が見る範囲は `obstacle_distance` に限られるので、経路のバウンディングボックスを `ceil(d0/res) + 1` セル広げた窓で距離変換すれば足りる。窓の外は `reach` を返すので項は働かない。これで実地図でも距離場が無視できる costになり、**既定で入れても計画時間は 144 → 105 ms と悪化しない**。

障害物項の重みは実測で選んだ。強くするとクリアランスは伸びるが経路が波打つ:

| `wo` / `d0` | 経路長 | p05 | 平均 | p99 \|k\| | 最大 \|k\| | 符号反転 |
|---|---|---|---|---|---|---|
| 0 (旧既定) | 37.246 m | 0.050 m | 0.256 m | 2.559 | 3.452 | 6 |
| **0.05 / 0.3 (新既定)** | 37.361 m | **0.132 m** | 0.292 m | 2.644 | **3.403** | 21 |
| 0.10 / 0.5 | 37.657 m | 0.271 m | 0.389 m | **8.046** | **17.680** | 53 |

新既定は **p99 と最大曲率を悪化させずにクリアランスを 2.6 倍**にする。符号反転は増えるが振幅が極小の揺れで、F-17 が指標にした p99 / 最大は動いていない。`0.10 / 0.5` は曲率が崩れるので採らない。
...
### 14.7 向きを見て膨張帯に入る (狭路の本命)

§14.6 で「帯を開けると衝突する」と書いたが、これは**向きを捨てているからそうなる**のであって、帯そのものが危険なのではない。`Circumscribed` の定義は「基準点がそこにあるとき**向きによっては**衝突する」であり、Hybrid A\* は向きを持っている。使えばよい。

分類グリッドの 1 byte に、分類 (2 bit) に加えて**そのセルが実際の障害物か** (1 bit) を載せた。`is_obstacle()` は膨張済み地図でも元の障害物を指す (膨張が書くのは最大 253、`LETHAL_OBSTACLE` は 254)。`CollisionRadii` にも `is_obstacle()` を足したので、距離場モデルでも同じことができる。**探索コアからは相変わらず `Map` も `Model` も見えないので、§2 の型消去の縫い目はそのままである。**

`HybridAStarParams::footprint` に車体外形を与えると、通行可否は 3 段になる。

| 中心セルの分類 | 判定 |
|---|---|
| `Free` | そのまま通す (外形を見ない。ここが性能の要) |
| `Circumscribed` | **その姿勢の外形**が実障害物セルの中心を覆うかを厳密に判定 |
| `Inscribed` | 不可 (内接円が当たるので、どの向きでも衝突する) |

これは `collision::check_footprint()` の 2 段構えと同じ方針を、探索の内側に置いたものである。successor の終端・primitive 内部のサンプル・解析接続 (Dubins / Reeds-Shepp) のすべてが同じ判定を通る。

実測 (Kachaka: 0.387 × 0.240 m、原点が前寄り、`inscribed 0.120` / `circumscribed 0.266`。`Free` が繋がるには 0.53 m 幅が要る):

| 通路幅 | 外形なし | 外形あり |
|---|---|---|
| 0.20 m (車体 0.24 m より狭い) | `Unreachable` | **`Unreachable`** (通れないものは通さない) |
| 0.30 m | `Unreachable` | **経路あり・フットプリント衝突 0** |
| 0.50 m | `Unreachable` | 経路あり・衝突 0 |
| 0.60 m | 経路あり・衝突 0 | 経路あり・衝突 0 |

**`Free` しか通らない設計では 0.53 m 未満がすべて壁になっていた。** 車体は 0.24 m なので、その差はまるごと「通れるのに通らない」領域だった。外形を与えると 0.30 m まで下がり、しかも厳密判定なので衝突は 0 である。開けた地図では外形の有無で経路がビット同一になる (帯に入らないので判定が呼ばれない)。

**corridor を切り出す guide は別問題である。** Hybrid A\* を corridor 上で走らせる構成では、corridor を決める A\* が先に `Unreachable` で落ちる。A\* は向きを持たない (yaw は接線) ので同じ手が使えない。guide は**探索範囲を決めるだけで追従しない**ので、`narrow_passage.enabled = true` で帯込みで走らせ、安全性は Hybrid A\* の外形判定に担わせるのが役割分担として正しい。`eltanin_ros` の `plan_attempt.cpp` はこの形にしてある。

### 14.8 その場旋回の掃引を検査していなかった

`interior_is_free()` は「その場旋回は基準点が動かないので検査するものはない」というコメント付きで、旋回を無条件に通していた。**基準点は動かなくても車体は回る。** 外形判定 (§14.7) を入れるまでこの前提は分類と整合していた (帯に入れないので旋回できる場所しか通らない) が、帯を開けた瞬間に破綻した。

`attach_analytic_expansion()` が末尾に足す「ゴール向きへの旋回」も無検査だった。解析接続の受理前に掃引するようにした。

幅 0.40 m のくぼみ (車体 0.387 × 0.240 m、外接円直径 0.53 m) での実測:

| 能力 | 修正前 | 修正後 |
|---|---|---|
| `turn_in_place` のみ | **2.16 m の経路を返す** (物理的に回れない) | `Unreachable` |
| `reverse` のみ | 2.67 m、後退 28 | 変化なし |
| 両方 | 2.16 m (同上) | **2.55 m、後退 28 + 旋回 3** |

外形判定を入れたから見つかった穴である。掃引の角度刻みは `SPIN_CHECK_STEP = 0.05 rad`。

### 14.9 膨張帯に値段を付けた

外形判定は「入れるか」しか答えない。**入ってよいかは別の問題である。** 帯は衝突しないための安全余裕なので、余裕があるなら使うべきではない。ところが通行を許可した途端、探索は最短だからと帯を常用した。

実機のログ (182×295 @ 0.025 m):

```
path 4.41 m, 126 poses: free 76, circumscribed 50, inscribed 0
```

126 姿勢中 50 (40 %) が帯である。`inscribed 0` なので実障害物には触れていないが、車体中心から壁までが **0.121 m** まで詰まった経路が出た。内接円半径 0.120 m に対して余裕 1 mm である。

`circumscribed_penalty` (既定 6.0) は帯を 1 m 通るごとに 6 m 分を上乗せする。迂回できるなら Free 側を通り、迂回路がない狭所では従来どおり通る (合成の 0.30 m ドアで 0〜12 に振っても通行本数は不変)。実機では帯が 40 % → 25 % に下がった。

**`TraversabilityPolicy::circumscribed_penalty` と二重定義していた**のを統合したものである。どちらも「膨張帯を通るコスト」だった。

### 14.10 解析接続がコストを一切見ていなかった

解析接続 (Dubins / Reeds-Shepp のテール) は「成功したら即 return」だったため、**探索側に付けたコストがテールに一切適用されていなかった**。短い経路はほぼ全区間がテールなので、帯を避ける仕組みが経路の大部分に効いていなかった。

三段階で直した。

1. **即採用をやめ、コスト付きの候補にした。** ヒューリスティックが下界なので、キュー最小の `f` が候補コストを超えた時点で確定できる。最適性はむしろ改善する
2. `reverse_penalty` を反映させた。以前は `reverse_penalty = 1000` にしても 2 m の後退テールが返っていた。いまは Dubins 接続と Reeds-Shepp 接続のペナルティ込みコストを比較して安い方を先に試す
3. `circumscribed_penalty` を反映させた (§14.9)

**クリアランス設定を何倍にしても経路が 1 mm も変わらなかったのはこれが原因である。**

### 14.11 帯に入るのは最後の手段にした

外形判定は安くない。実機相当の地図で計画 1 回が 232 → 580 ms になった。しかも Free だけで解ける経路まで帯を通るようになり、**安全余裕を使い切った直線**が出た。

Free セルだけで先に解き、解けなかったときだけ外形を出す構成にした。通常の計画は今日の作業前と同じ探索になり、狭路だけが外形判定を通る。

| 構成 | 合計 |
|---|---|
| 今日の作業前 | 232 ms |
| 帯に常時入る | 580 ms |
| 帯は最後の手段 + A\* クリアランス | **199 ms** |

**2 パスのクリアランス** (強い設定で試し、遠回りが 1.25 倍を超えたら弱い設定で再試行) も入れたが、1154 ms かかったので既定を切にした。効果に対して代償が大きい。

### 14.12 終端の旋回は出力しない選択肢を付けた

`turn_in_place` を許すと、経路の末尾が「移動ゼロ・yaw だけ変わる」姿勢になる。`navyu_path_tracker` は次の点へ向かう制御なので**この点を追えず、ゴール周辺を回り続ける**。§13.15 で `Differential` を入れたときからの仕様で、D-9 として申し送っていた項目が実機で顕在化した。

`emit_goal_rotation = false` で、最後に動く姿勢の yaw をゴール向きに畳む。**妥協である**: 畳んだぶん末尾で yaw が飛ぶので運動学的に連続ではなく、最終合わせは追従側任せになる。経路の途中に出る旋回は畳まないので、同じ症状が途中で出る余地は残る。

本筋は追従側が「間隔 0 の区間」と後退区間を扱えるようにすることである。

### 14.13 名前を nav2 の語彙に寄せた

| 変更前 | 変更後 | 理由 |
|---|---|---|
| `ObstacleField` (クラス) | `ClearanceMap` = `map::DistanceMap` | 距離に関する型が 2 つあるのを 1 つにした。補間と勾配は自由関数 |
| `CollisionRadii` | `DistanceTraversabilityModel` | `classify()` を持つのに名前が半径しか言っていなかった。`CostTraversabilityModel` と対になる |
| `InflationCostModel::radii()` | `distance_model()` | `DistanceTraversabilityModel` を返すのに `radii()` は食い違っていた |
| `band_penalty` | `circumscribed_penalty` | 「band」は eltanin 固有。二重定義も統合 (§14.9) |
| `surcharge()` | `extra_cost()` | 「追加料金」は制御では使わない |
| `TraversabilityView::blocked()` | `is_obstacle()` | `ObstacleModel::is_obstacle()` と同義なのに別名だった |
| `pose_is_usable()` / `footprint_fits()` | `pose_is_free()` / `footprint_is_clear()` | nav2 は `isNodeValid` / `inCollision` |
| `tail_cost()` / `attach_tail()` / `connect_goal()` | `analytic_expansion_cost()` / `attach_analytic_expansion()` / `try_analytic_expansion()` | nav2 は **analytic expansion** と呼ぶ |
| `NarrowPassageFallback` | `TraversabilityFallback` | 地形ではなく通行判定の緩和である |
| `PlanResult::narrow_passage()` | `relaxed()` | `TraversabilityView::relaxed()` と揃う |
| `emit_final_turn` | `emit_goal_rotation` | 何の turn か言っていなかった |

`penalty` は nav2 の語彙 (`reverse_penalty` / `change_penalty` / `cost_penalty`) なので残した。

`PlanResult::mark_narrow_passage()` は**廃止**した。語の問題以前に、不変であるべき型に setter が生えていたのが誤りである。生成時に渡す。

あわせて `Forward-only Hybrid A*` という doc を消した。Reeds-Shepp 対応後は**嘘**になっていた。

### 14.14 既定値で三度失敗した

いずれも「狙ったケースでは動く」ことだけ確認し、**通常のケースで何が変わるかを測らずに既定値を決めた**ものである。

| # | 入れたもの | 起きたこと |
|---|---|---|
| 1 | 狭路フォールバックを既定 `true` | フットプリントが壁を突き抜ける経路が実機に出た。`Circumscribed` は「向きによっては衝突する」であって通れる保証ではない |
| 2 | クリアランス `{1.0, 0.6}` | 統合デモで障害物脇の隙間を避けて 59 m の大回りをし、`replan_limit` に達した |
| 3 | 外形判定を常時適用 | Free だけで解ける経路まで帯を通り、安全余裕を使い切った直線が出た |

**既定値は、狙ったケースと通常のケースの両方を測ってから決める。** 1 については、確認を取る際に論点を「膨張帯のゴールに到達できるか」としてしまい、「黙って衝突しうる経路を返す」点を示さなかった。既定値の安全性は確認を取る前に自分で検証すべきだった。

診断のために `eltanin_ros` の `global_path_planner` へ、返した経路が立つセルの分類・実フットプリント衝突数・最短壁距離をログ出力させた。**目視での往復をやめ、数字が出てから 3 手で原因に到達した。**

### 14.15 申し送り

- ~~**追従側が経路を実行できない。**~~ **後退は T13 で埋まった** (`docs/control-design.md` §14)。`MpcFollower` は run 単位でカスプを実行し、`PurePursuit` は後退区間を含む経路を `PathNotSupported` として拒否する。**残るのは経路途中のその場旋回だけ**で、`Direction::InPlace` は表現できるが能動的に回る制御モードは無い。`emit_goal_rotation = false` が末尾だけの妥協である点も変わらない
- ~~**経路が区間の種別を持っていない。**~~ **T13 で `eltanin::Direction` と `Path` のセグメント方向配列が入った。**`reconstruct_poses()` は `Node::mode` を、解析接続は Reeds-Shepp の run の符号をそのまま載せる。追従側は幾何から推測しない。ワイヤ上は `eltanin_msgs/DirectedPath` が運ぶ
- **A\* のクリアランス整形を既定で入れるには**、探索が触る範囲を先に絞る必要がある。ガイド経路の窓で 2 段階に走らせる案は 190 + 10 + 240 ≈ 440 ms でまだ既定より遅い。距離変換そのもの (16M セルで 461 ms) を速くする方が筋が良い。**なお ROS の global costmap は 182×295 なので、そちらでは既定で入れてある**
- Reeds-Shepp は解析接続と制御集合に入り、**`MpcFollower` が後退区間を解釈する** (T13)。**`PurePursuit` は据え置きで、拒否するだけ**である — `ω = 2 v sinα / L` は前進でのみ安定な法則で、後退版は別の制御則になる。間隔 0 の区間 (その場旋回) についての `.plait/03-design-review.md` の D-9 はそのまま残っている
- 障害物項は局所勾配なので、狭所では両側から押されて揺れる。Dolgov の共役勾配法のように全体最適で解けば揺れずに離せる
- **A\* には外形判定を入れられない。** 向きを持たないので、狭路では guide としてしか使えない。A\* 単独で狭路を通したいなら Theta\* のような任意角探索か、向きを状態に持つ別のプランナが要る
- 外形判定はセル中心が外形の内側にあるかで見ている。セルの角だけが外形に入る配置は見逃すので、`resolution` が車体に対して粗いと甘くなる
- **corridor の幅が Hybrid A\* の逃げ場を決めている。** `corridor_margin_cells = 30` は 0.025 m 地図で ±0.75 m しかない。guide が壁沿いを通れば corridor も壁沿いになるので、guide 側にもクリアランスを効かせてある。それでも足りない場合は margin を広げるしかない
- 外形判定は `Circumscribed` セルでのみ走るが、そこでは bounding box 内の全セルを走査する。`resolution` が細かいほど重い (0.025 m で 16×16 セル)。(cell, heading_bin) 単位のキャッシュか、粗いブロックでの事前判定で削れる
